...
class BeneficiariosTransferScreen extends StatelessWidget {
  ...
  const BeneficiariosTransferScreen({super.key});
        ...
        children: [
          ...
          BlocBuilder<BeneficiariosBloc, BeneficiariosState>(
            builder: (context, state) {
              return Expanded(
                child: ListView(
                  children: state.beneficiarios
                      .map((b) => _productCard(context, b))
                      .toList(),
                ),
              );
            },
            ...
            builder: (context, state) {
              ...
              return Padding(
                ...
                child: ElevatedButton(
                  style: selectedBeneficiario == null
                      ? ElevatedButton.styleFrom(
                          backgroundColor: color0080F22E
                        )
                      : null,
                  onPressed: selectedBeneficiario == null
                      ? () {
                          context.pushNamed(CreateBeneficiaryScreen.routeName);
                        }
                      : () {
    ...
    return BlocBuilder<BeneficiariosBloc, BeneficiariosState>(
      builder: (context, state) {
        ...
        return Stack(
          children: [
            Padding(
              padding: EdgeInsets.symmetric(horizontal: 21.w(context)).copyWith(
                top: 12.h(context),
              ),
              child: GestureDetector(
                onTap: () {
                  ...
                  if (selected?.id == beneficiario.id) {
                    context
                        .read<BeneficiariosBloc>()
                        .add(ClearBeneficiarioSelectionEvent());
                  } else {
                    context
                        .read<BeneficiariosBloc>()
                        .add(SelectBeneficiarioEvent(beneficiario));
                  }
                },
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                        color: beneficiario.id == selected?.id
                            ? color005199
                            : colorDCE2E8),
                  ),
                  child: Column(
